...
class TestQueryFlow:
    ...
    def test_query_returns_answer_after_indexing(
        ...
        with patch.object(engine, "_build_index", return_value=True):
            ...
            mock_query_engine.query.return_value = (
                "This document is about a RAG system for testing."
            )
...
class TestChatSessionState:
    ...
    def test_multiple_queries_maintain_context(
        self, tmp_data_dir, tmp_chroma_dir, sample_txt_file
    ):
        ...
        with patch.object(engine, "_build_index", return_value=True):
            ...
            mock_query_engine.query.side_effect = [
                "I am a RAG system.",
                "I just told you, I am a RAG system.",
            ]
            engine._index.as_query_engine.return_value = mock_query_engine
...
class TestErrorHandling:
    ...
    def test_query_with_no_index_shows_helpful_error(
        self, tmp_data_dir, tmp_chroma_dir
    ):
        engine = RAGEngine(data_dir=str(tmp_data_dir), chroma_dir=str(tmp_chroma_dir))

        with patch.object(
            engine, "_build_index", side_effect=ValueError("No documents found")
        ):
            with pytest.raises(ValueError, match="No documents found"):
                engine.query("Hello")

    def test_query_when_ollama_down_shows_error_in_response(
        self, tmp_data_dir, tmp_chroma_dir, sample_txt_file
    ):
        engine = RAGEngine(data_dir=str(tmp_data_dir), chroma_dir=str(tmp_chroma_dir))
